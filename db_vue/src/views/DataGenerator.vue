<template>
    <div class="data-generator">
        ...
        <div class="left-panel">
            ...
            <button
                @click="generateData"
                :class="{ 'button-disabled': !params.dataset_name }"
                :disabled="!params.dataset_name"
            >
                生成数据
            </button>
        ...
        <div class="right-panel">
            ...
            <div class="realtime-monitor">
                <h2>数据生成监控区</h2>
                <p v-if="monitoringData == 3">正在生成数据...请耐心等待...</p>
                ...
                <p v-else-if="monitoringData == 4">负载正在生成中，请稍等...</p>
                <p v-else-if="monitoringData == 5">负载生成成功！</p>
                <p v-else-if="monitoringData == 6">负载生成失败！</p>
...
export default {
    data() {
        return {
            ...
            // csv文件属性
            sqlcsvFiles: [], // 存储上传的SQL文件
            sqlFileInputPlaceholder: "请选择.csv查询文件", // SQL文件上传占位符
        };
    },
    computed: {},
    created() {
    ...
    methods: {
        ...
        handleSQLFileUpload3(event) {
            // 处理SQL文件上传
            const sqlcsvFile = event.target.files[0];
            if (sqlcsvFile) {
                this.sqlcsvFiles.push(sqlcsvFile);
                console.log("SQL File added:", sqlcsvFile.name);
        ...
        generateLoad() {
            ...
            // 触发后端API逻辑
            if (this.sqlcsvFiles.length > 0) {
                let formData = new FormData();
                formData.append("sql_file", this.sqlcsvFiles[0]);

                axios
                    .post(
                        // "http://127.0.0.1:8000/generator/generate-sql/",
                        "http://172.31.178.221:8000/generator/generate-sql/",
                        formData,
                        {
                            headers: {
                                "Content-Type": "multipart/form-data",
                    ...
                    .then((response) => {
                        if (response.data.status == "success") {
                            this.monitoringData = 5;
                        } else if (response.data.status == "error") {
                            this.monitoringData = 6;
                        }
                        console.log(
        ...
        generateData() {
            ...
            this.params.dataset_name = "";
            // 发送 POST 请求到服务器以生成数据
            axios
                .post(
                    // "http://127.0.0.1:8000/generator/generate-data/",
                    "http://172.31.178.221:8000/generator/generate-data/",
                    formData,
                    {
                        headers: {
                            "Content-Type": "multipart/form-data",
